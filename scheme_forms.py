...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        print("DEBUG:", "signature is", signature, "symbol is", expressions.rest.first)
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 14
    while bindings:
        print("DEBUG:", "bindings is", bindings, "bindings.first is", bindings.first)
        validate_form(bindings.first, 2, 2)
        symbol = bindings.first.first
        names = Pair(symbol, names)
        expression = eval_all(bindings.first.rest, env)
        vals = Pair(expression, vals)
        validate_formals(names)
        print("DEBUG:", "names is", names, "vals is", vals)
